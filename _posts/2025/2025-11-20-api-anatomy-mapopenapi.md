...
The `AddOpenApi` call registers services that are required for OpenAPI document generation, including a set of providers that understand how to examine the endpoints in the app and derive structured descriptions for them (see [this earlier blog post I wrote]({% post_url /2024/2024-05-27-openapi-in-aspnetcore %})). The `MapOpenApi` method registers an endpoint that emits the OpenAPI document in JSON format. By default, the document is served at `http://localhost:{port}/openapi/v1.json`, where `v1` is the default document name. The document that you get is rich with metadata about the parameters and responses that this API produces.
