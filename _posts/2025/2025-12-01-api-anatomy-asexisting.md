...
Now for the part where I air some grievances. The `PublishAsExisting` name has proven to be a brittle choice. In [one of my earlier posts]({% post_url /2025/2025-10-06-aspire-publish-vs-deploy %}), I talked about how the concept of run-mode and publish-mode is becoming blurred in Aspire, especially as we try to bring the model of the application running in the cloud and the one running locally as close to each other as possible. Encoding a now-evolving verb into the API name is...annoying.
